...
void Convertion::convert(std::vector<GifFrame> frames, std::string output, GifSettings settings, std::string extension)
{
    ...
    if(settings.m_is_transparent){
        ...
        batchFile << "ffmpeg -f concat -safe 0 -i files.txt -vf \"pad="<<settings.m_width<<":"<<settings.m_height<<":(ow-iw)/2:(oh-ih)/2\" -t 15 " << loop_option << output << std::endl;
    }
    for (size_t i = 0; i < frames.size(); ++i) {
    ...
    if(settings.m_is_open_folder){
        batchFile << "explorer " << output << std::endl;
    }
