...
void Convertion::convert(std::vector<GifFrame> frames, std::string output, bool has_pallete, std::string extension)
{
    ...
    if(has_pallete){
        ...
        batchFile << "ffmpeg -f concat -safe 0 -i files.txt -vf \"pad=388:377:(ow-iw)/2:(oh-ih)/2\" -t 15 " << output << std::endl;
    }
    for (size_t i = 0; i < frames.size(); ++i) {
