...
void GameScene::init() {
  ...
  m_gifsettings_handler =
      std::make_unique<Fini>("res/data/gif_img_settings.ini");
  ...
  m_gifsettings_handler->initialize_value("gif settings", "open_folder",
                                          "true");

  m_gifsettings_handler->save();

  m_gifsettings_handler->grab_value("gif settings", "png",
                                    &m_gif_settings.m_is_transparent);
  m_gifsettings_handler->grab_value("gif settings", "looping",
                                    &m_gif_settings.m_is_looping);
  m_gifsettings_handler->grab_value("gif settings", "open_folder",
                                    &m_gif_settings.m_is_open_folder);
...
void GameScene::update(double deltaTime) {
  ...
  m_gifsettings_handler->set_value("gif settings", "png",
                                   m_gif_settings.m_is_transparent);
  m_gifsettings_handler->set_value("gif settings", "looping",
                                   m_gif_settings.m_is_looping);
  m_gifsettings_handler->set_value("gif settings", "open_folder",
                                   m_gif_settings.m_is_open_folder);

  if (Mouse::is_at_area({m_preview_button->relative_x,
                         m_preview_button->relative_y, 25, 25}) ||
      Mouse::is_at_area({m_convert_button->relative_x,
                         m_convert_button->relative_y, 25, 25})) {
    m_is_hovering = true;
  } else {
    m_is_hovering = false;
  }
  ...
  if (Mouse::is_at_area(
          {m_return_button->relative_x, m_return_button->relative_y, 25, 25})) {
  ...
  m_left_door_x =
      Math::lerp(m_left_door_x, m_left_door_pos, m_door_speed * deltaTime);
  m_right_door_x =
      Math::lerp(m_right_door_x, m_right_door_pos, m_door_speed * deltaTime);
  ...
  if (Mouse::is_at_area({m_convert_button->relative_x,
                         m_convert_button->relative_y, 25, 25})) {
    ...
    if (m_is_mouse_down) {
      ...
      if (path != "") {
        Convertion::convert(m_video_frames, path, m_gif_settings,
                            m_current_extension);
      }
...
void GameScene::ui() {
  {
    ...
    if (m_frame_hover_tooltip) {
      ...
      ImGui::Text("Frames: %d",
                  m_timeline.myItems[m_hover_frame].mFrameEnd -
                      m_timeline.myItems[m_hover_frame].mFrameStart);
      ImGui::Text(
          "Duration in seconds : %f",
          static_cast<float>((m_timeline.myItems[m_hover_frame].mFrameEnd -
                              m_timeline.myItems[m_hover_frame].mFrameStart)) /
              m_gif_settings.fps);
      ImGui::EndTooltip();
    }
    ...
    if (ImGui::Begin("Timeline", nullptr,
      ...
      // For example:
      Sequencer(
          &m_timeline, &currentFrame, &expanded, &selectedEntry, &firstFrame,
          ImSequencer::SEQUENCER_DEL | ImSequencer::SEQUENCER_CHANGE_FRAME |
    ...
    if (ImGui::Begin("Settings", nullptr,
                     ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize)) {
      ImGui::Checkbox("Transparent background",
                      &m_gif_settings.m_is_transparent);

      if (ImGui::IsItemHovered()) {
        ImGui::BeginTooltip();
        ImGui::Text("WARNING: All the images need to be the same size to work "
                    "with transparent background!");
        ImGui::EndTooltip();
      }

      ImGui::Checkbox("Loop", &m_gif_settings.m_is_looping);
      ImGui::Checkbox("Open folder after convertion",
                      &m_gif_settings.m_is_open_folder);
      ImGui::End();
    }
    ImGui::PopStyleColor();
    ...
    if (ImGui::BeginPopupModal("Edit Frame", NULL,
                               ImGuiWindowFlags_AlwaysAutoResize)) {
      ImGui::Text("Edit the frame");
      ImGui::InputInt("Frame Start",
                      &m_timeline.myItems[selectedEntry].mFrameStart);
      ImGui::InputInt("Frame End",
                      &m_timeline.myItems[selectedEntry].mFrameEnd);
...
void GameScene::draw() {
  ...
  if (frame != nullptr) {
    m_atlas->draw(frame->m_texture, {256, 192}, 1,
                  Math::mid(m_app->get_window_size().x, 256) - 18, 10, false);
  }
  ...
  if (m_is_playing)
    m_atlas->draw(20, 20, std::to_string(m_current_frame).c_str(),
                  m_app->get_main_font(), {255, 255, 255});

  // gizmos stuff
  Gizmos::draw_area(
      {m_preview_button->relative_x, m_preview_button->relative_y}, 25, m_atlas,
      {255, 0, 0});
  Gizmos::draw_area(
      {m_convert_button->relative_x, m_convert_button->relative_y}, 25, m_atlas,
      {255, 0, 0});
  Gizmos::draw_area({m_return_button->relative_x, m_return_button->relative_y},
                    25, m_atlas, {255, 0, 0});
...
void GameScene::input(SDL_Event event) {
  ...
  case SDL_DROPFILE:
    ...
    SequencerItemTypeNames[m_current_file_idx] =
        filenames_storage.back().c_str();
    m_timeline.myItems.push_back(Timeline::MySequenceItem{
        m_current_file_idx,
        0 + static_cast<int>(m_gif_settings.fps * m_timeline.myItems.size()),
        m_gif_settings.fps +
            static_cast<int>(m_gif_settings.fps * m_timeline.myItems.size()),
        false});

    m_video_frames.push_back(
        GifFrame{event.drop.file, get_filename(event.drop.file),
                 m_resources->LoadTexture(event.drop.file)});

    int width, height;

    SDL_QueryTexture(m_video_frames.back().m_texture, nullptr, nullptr, &width,
                     &height);
...
void GameScene::clean() { m_gifsettings_handler->save(); }
