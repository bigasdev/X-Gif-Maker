...
void MainScene::init() {
  // m_cd->set_state("init_event", .1f, [&] {	m_logger->log("Starting the
  // MainScene!");});
  ...
  // gif settings initialize
  {
    ...
    m_fgif_settings->initialize_value("gif_settings", "folder", m_folder_path);
    ...
    m_fgif_settings->grab_value("gif_settings", "folder", &m_folder_path);
  }
...
void convert_file(std::string file) {
  ...
  std::string command = "-w " + std::string(m_selected_width) + " -f " +
                        m_selected_fps + " -q " + m_selected_quality +
                        " -o \"" + m_folder_path + "\\" +
                        filenameWithoutExtension + ".gif\""; // Default command

  // std::string command = "-w 1024 -q 6 -o \"" + m_folder_path + "\\" +
  // filenameWithoutExtension + ".gif\""; // Default command
...
void MainScene::update(double deltaTime) {
  ...
  // buttons
  {
    if (Mouse::is_at_area({data_convert_button->relative_x,
                           data_convert_button->relative_y, 156, 78})) {
    ...
    if (Mouse::is_at_area({data_remove_file->relative_x,
                           data_remove_file->relative_y, 50, 50})) {
      if (m_current_mouse_key == LEFT_CLICK) {
        if (m_file_path != "") {
          m_file_path = "";
        } else {
    ...
    if (Mouse::is_at_area({data_select_folder->relative_x,
                           data_select_folder->relative_y, 24, 24})) {
      if (m_current_mouse_key == LEFT_CLICK) {
        m_folder_path = Data_Loader::load_folder("Select a folder");
        m_current_mouse_key = NO_KEY;
      }
    }
    if (Mouse::is_at_area({change_game_scene->relative_x,
                           change_game_scene->relative_y, 24, 24})) {
      if (m_current_mouse_key == LEFT_CLICK) {
  ...
  m_fgif_settings->set_value("gif_settings", "folder", m_folder_path);
}
...
void status_bar_ui() {
  ...
  ImGui::Begin("Status Bar UI", NULL,
...
void base_ui() {
  ...
  ImGui::SetNextWindowPos(
      ImVec2(middle_of_screen.first, middle_of_screen.second));
  ImGui::Begin("Main Scene", NULL,
               ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
                   ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
  if (is_convertion_running) {
...
void video_settings() {
  ImGui::SetNextWindowPos(
      ImVec2(data_settings->relative_x, data_settings->relative_y));
  ImGui::SetNextWindowSize(ImVec2(200, 600));
  ImGui::Begin("Video Settings", NULL,
               ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
                   ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
  ...
  if (ImGui::BeginCombo(
          "##select_width",
          m_selected_width)) // The second parameter is the label previewed
                             // before opening the combo.
  {
    for (int n = 0; n < IM_ARRAYSIZE(width); n++) {
      bool is_selected = (m_selected_width ==
                          width[n]); // You can store your selection however you
  ...
  if (ImGui::BeginCombo(
          "##select_quality",
          m_selected_quality)) // The second parameter is the label previewed
                               // before opening the combo.
  {
    for (int n = 0; n < IM_ARRAYSIZE(quality); n++) {
      bool is_selected =
          (m_selected_quality ==
           quality[n]); // You can store your selection however you want,
  ...
  ImGui::Text("Gif FPS:");
  if (ImGui::BeginCombo("##select_fps",
                        m_selected_fps)) // The second parameter is the label
                                         // previewed before opening the combo.
  {
    for (int n = 0; n < IM_ARRAYSIZE(fps); n++) {
      bool is_selected = (m_selected_fps ==
                          fps[n]); // You can store your selection however you
                                   // want, outside or inside your objects
...
void MainScene::draw() {
  ...
  // m_app->get_atlas()->draw(m_add_symbol_tx, vec2f(44, 44), 1, 544, 517,
  // false);

  // tutorial texts
  {
    m_atlas->draw(data_press_e->relative_x, data_press_e->relative_y,
                  "Select file", m_app->get_main_font(), {255, 255, 255, 125});
    m_atlas->draw(data_press_f->relative_x, data_press_f->relative_y,
                  "Choose destination", m_app->get_main_font(),
                  {255, 255, 255, 125});
  }
  ...
  Gizmos::draw_area(
      ...
      m_atlas, {255, 0, 0});
  Gizmos::draw_area(
      vec2f(data_remove_file->relative_x, data_remove_file->relative_y), 50,
      m_atlas, {255, 0, 0});
  Gizmos::draw_area(
      vec2f(change_game_scene->relative_x, change_game_scene->relative_y), 24,
      m_atlas, {255, 0, 0});

  if (m_folder_path != "") {
    std::string folder_name =
        m_folder_path.substr(m_folder_path.find_first_of("\\") + 1, 38);

    m_atlas->draw(data_folder_name->relative_x, data_folder_name->relative_y,
                  folder_name.c_str(), m_app->get_main_font(),
                  {255, 255, 255, 255});
  }
  ...
  if (m_file_path != "") {
    ...
    m_atlas->draw(data_file_name->relative_x, data_file_name->relative_y,
                  filenameWithoutExtension.c_str(), m_app->get_main_font(),
                  {255, 255, 255, 255});
  }

  if (!is_convertion_running) {
    m_atlas->draw(m_resources->GetAsset("ready_bar")->GetTexture(),
                  vec2f(269, 16), 3, 0, 0, false);
  } else {
    m_atlas->draw(m_resources->GetAsset("stop_vfx")->GetTexture(),
                  vec2f(268, 20), 3, 0, 0, false);
    m_atlas->draw(m_resources->GetAsset("stop_bar")->GetTexture(),
                  vec2f(259, 4), 3, 12, 15, false);
  }

  // convertion state
  m_atlas->draw(data_convertion_state->relative_x,
                data_convertion_state->relative_y,
                is_convertion_running ? "Converting..." : "READY TO CONVERT",
                m_app->get_main_font(), {255, 255, 255, 255});

  // file and folder feedback
  m_atlas->draw(m_folder_path == ""
                    ? m_resources->GetAsset("folder")->GetTexture()
                    : m_resources->GetAsset("folder_selected")->GetTexture(),
                vec2f(20, 15), 2, data_folder->relative_x,
                data_folder->relative_y, false);
  m_atlas->draw(
      m_file_path == "" ? m_resources->GetAsset("file")->GetTexture()
                        : m_resources->GetAsset("file_selected")->GetTexture(),
      vec2f(22, 25), 2, data_file->relative_x, data_file->relative_y, false);

  // Gizmos::draw_line(vec2f(50, 50), vec2f(200, 200), m_app->get_atlas(),
  // {255,0,0}); Gizmos::draw_circle(vec2f(200, 200), 50, m_app->get_atlas(),
  // {255,0,0}); Gizmos::draw_circle(m_test_entity->get_pos(), 15,
  // m_app->get_atlas(), {0,255,0});
  for (auto &scene : m_partial_scenes) {
    scene->draw();
  }
}
