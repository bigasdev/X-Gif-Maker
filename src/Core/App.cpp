...
void App::render() {
  ...
  if (m_is_loaded) {
    ...
    m_atlas_ptr->draw((m_window_size.x - 70) / 2, (m_window_size.y - 50) / 2,
                      "Loading...", s_main_font, {255, 255, 255, 255});
  }
...
void App::set_fps(float fps) { m_fps = Math::clamp(0, fps, 60); }
...
IniHandler *App::get_ini_handler() { return m_ini_handler_ptr; }
